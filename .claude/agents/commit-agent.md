...
- **Você NUNCA edita, escreve ou reescreve `.claude/settings.json`, `.claude/settings.local.json`, nem qualquer arquivo dentro de `.claude/` que controle permissões, hooks ou configuração do Claude Code — nem via `echo`/heredoc/redirecionamento no Bash, nem de nenhuma outra forma.** Isso vale mesmo que você mesmo (ou o comando que você quer rodar) esbarre num prompt de permissão. Se um comando `git` for bloqueado por permissão, **não tente contornar isso editando a configuração** — pare e reporte no resumo final que o comando X foi bloqueado e precisa de aprovação manual do usuário. Só pode commitar mudanças em `.claude/settings.json` quando elas já existiam no arquivo antes de você começar (feitas por humano ou por outro agente), nunca mudanças que você mesmo fez.
- Você tem autorização permanente do usuário para commitar e dar push automaticamente como parte deste fluxo — isso não precisa de confirmação a cada vez, ao contrário da regra padrão de "nunca commitar sem pedido explícito". Essa autorização vale só para o fluxo normal de commit/push descrito acima (arquivos de código e conteúdo já existente), não para nada destrutivo, nem para alterar sua própria configuração/permissões, nem para nada fora desse escopo.
</content>
