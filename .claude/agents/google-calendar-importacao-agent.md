...
## Quarto bug real corrigido aqui (2026-08-07) — não reintroduza (só lia a agenda "primary")

**Sintoma relatado pelo usuário**: comparando a mesma semana no Google Calendar (visão semanal, múltiplas agendas sobrepostas: "Patricia Grando Müller"/primary, "Aniversários", "CONSULTÓRIO", "Horários marcados no Facebook", "PsicoManager", "Tarefas", "Holidays in Brazil") contra a tela "Agenda & Consultas" do app, **dezenas de compromissos existiam no Google mas não apareciam no app** — mesmo com "Sincronismo: Dados atual..." no topo e sem erro visível.

**Causa raiz**: `importGoogleCalendarEventsToAppointments()`, `listGoogleManagedCalendarEvents()` e `listGoogleCalendarEvents()` sempre chamavam `events.list` com `calendarId: 'primary'` fixo. Isso lê só a agenda principal da conta — qualquer compromisso criado ou existente em uma agenda **secundária/compartilhada/assinada** do Google (ex.: "CONSULTÓRIO", "PsicoManager", "Horários marcados no Facebook") nunca era buscado, mesmo aparecendo normalmente na visão semanal do Google Calendar (que mostra todas as agendas marcadas de uma vez). Como o `maxResults`/paginação (bug 3) já estava correto, o sintoma não era "cortado em 2500" — era "nunca pediu" essas agendas.

Segundo problema, encadeado: o escopo OAuth solicitado (`GOOGLE_CALENDAR_SCOPES`) era só `calendar.events`, que não dá acesso a `calendarList.list` (necessário para descobrir quais outras agendas existem) — então mesmo corrigindo o código para percorrer múltiplas agendas, sem o escopo `calendar.calendarlist.readonly` a chamada de listagem de agendas falharia com "insufficient authentication scopes" para qualquer usuário já conectado antes desta correção.

**Correção**:
1. `GOOGLE_CALENDAR_SCOPES` agora inclui `https://www.googleapis.com/auth/calendar.calendarlist.readonly` além de `calendar.events`. Usuários já conectados precisam clicar em "Conectar Google Calendar" de novo (o botão já força `prompt: 'select_account consent'`, então basta um reconnect manual — não precisa reautorizar o projeto OAuth em si).
2. Novo método `listGoogleAppointmentCalendarIds()`: chama `gapi.client.calendar.calendarList.list()`, filtra agendas puramente informativas do Google que não são compromissos reais (feriados e aniversários de contatos, identificados pelo sufixo do `id` — `#holiday@group.v.calendar.google.com` / `#contacts@group.v.calendar.google.com` — não pelo nome exibido, que o usuário pode renomear) e agendas com `accessRole: 'freeBusyReader'` (só mostram ocupado/livre, sem detalhe de evento). Se a chamada falhar (ex.: usuário ainda não reconectou com o novo escopo), cai de volta silenciosamente para `['primary']` e loga um aviso — nunca quebra a importação inteira por causa disso.
3. `importGoogleCalendarEventsToAppointments()` agora itera `calendarIds` retornado por esse método, chama `fetchAllGoogleCalendarEvents` para cada uma, e faz merge deduplicando por `event.id` (um mesmo evento poderia, em tese, aparecer em mais de uma agenda consultada).
4. `listGoogleManagedCalendarEvents()` (lado do envio) e `listGoogleCalendarEvents()` (preview de "próximo evento") **continuam fixos em `calendarId: 'primary'` de propósito** — o envio só escreve/gerencia eventos na agenda principal (não faz sentido buscar eventos geridos por nós em agendas de terceiros), e o preview é só uma UI de status, não afeta dados. Só a importação precisava do escopo ampliado de leitura.

Sempre que investigar "faltam compromissos do Google" de novo, confirme antes de mais nada: (a) o compromisso ausente está em qual agenda do Google (não presuma que é sempre "primary"), e (b) se o usuário já reconectou o Google Calendar depois desta mudança de escopo — sem reconectar, `listGoogleAppointmentCalendarIds()` cai de volta para `['primary']` e o sintoma original volta a se manifestar mesmo com o código corrigido.
...
- **`calendarId` divergente entre os dois lados**: a partir da correção do bug 4, a importação percorre todas as agendas relevantes do usuário (`listGoogleAppointmentCalendarIds()`), enquanto o envio continua escrevendo/gerenciando eventos só em `calendarId: 'primary'`. Isso é intencional — não "conserte" o envio para também escrever em outras agendas sem pedido explícito do usuário, e não restrinja a importação de volta a `'primary'` sem entender que isso reintroduz o bug 4.
- **Escopo OAuth**: `GOOGLE_CALENDAR_SCOPES` inclui `calendar.events` (usado por ambos os lados para ler/escrever eventos) e `calendar.calendarlist.readonly` (usado só pela importação, para enumerar agendas). Se o envio precisar de um escopo adicional no futuro, adicione ao mesmo array em vez de criar uma segunda constante de escopo — os dois lados compartilham o mesmo `googleCalendarTokenClient`.
