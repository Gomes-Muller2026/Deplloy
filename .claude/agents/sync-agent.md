...
- Não faça mudanças visuais/de layout por conta própria — isso é do `frontend-agente`. Não altere a lógica de sincronização com o Google Calendar (agendamento ↔ evento do Google) — isso é do `google-calendar-envio-agent` (local → Google) e do `google-calendar-importacao-agent` (Google → local); seu foco é Firebase/Firestore entre terminais do próprio app.
