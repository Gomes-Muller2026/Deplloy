...
const APP_BRAND_NAME = 'Patricia Grando Muller';
const APP_BRAND_SUBTITLE = 'Psicoterapeuta';
...
const DEFAULT_WHATSAPP_BIRTHDAY_TEMPLATE = [
  ...
  'Que seu novo ciclo seja leve, com muita saúde e realizações.',
  '',
  'Com carinho,',
  '{{profissional}}'
].join('\n');
...
const parseDateBR = (dateStr) => {
  ...
  if (/^\d{8}$/.test(normalized)) {
    ...
    const possibleMonth = Number(normalized.slice(4, 6));
    const possibleDay = Number(normalized.slice(6, 8));

    if (firstFour >= 1900 && firstFour <= 2100 && possibleMonth >= 1 && possibleMonth <= 12 && possibleDay >= 1 && possibleDay <= 31) {
...
class ConsultorioApp {
  constructor() {
    ...
    this.agendaWeekOffset = 0;
    this.tabHistory = [];
    this.birthdayReminderDays = parseInt(localStorage.getItem('consultorio_birthday_reminder_days'), 10) || 7;
    this.whatsappTemplates = {
      appointmentConfirmation: localStorage.getItem('consultorio_whatsapp_template_confirmation') || DEFAULT_WHATSAPP_CONFIRM_TEMPLATE,
      birthdayGreeting: localStorage.getItem('consultorio_whatsapp_template_birthday') || DEFAULT_WHATSAPP_BIRTHDAY_TEMPLATE
    };
    this.pendingWhatsAppSend = null;
    this.lastGeneratedReportTitle = 'Relatório';
  ...
  initDOM() {
    ...
    const birthdayDaysInput = document.getElementById('birthday-reminder-days');
    const headerBirthdayDaysInput = document.getElementById('header-birthday-reminder-days');
    const whatsappConfirmTemplate = document.getElementById('whatsapp-template-confirmation');
    const whatsappBirthdayTemplate = document.getElementById('whatsapp-template-birthday');
    ...
    if (birthdayDaysInput) birthdayDaysInput.value = this.birthdayReminderDays;
    if (headerBirthdayDaysInput) headerBirthdayDaysInput.value = this.birthdayReminderDays;
    if (whatsappConfirmTemplate) whatsappConfirmTemplate.value = this.whatsappTemplates.appointmentConfirmation;
    if (whatsappBirthdayTemplate) whatsappBirthdayTemplate.value = this.whatsappTemplates.birthdayGreeting;
  ...
  bindDateInputMasks() {
    ...
    dateInputIds.forEach((id) => {
      ...
      input.addEventListener('input', (e) => {
        const masked = toDateInputMask(e.target.value);
      ...
      input.addEventListener('blur', (e) => {
        e.target.value = normalizeDateForInput(e.target.value);
        const parsed = parseDateBR(e.target.value);
        const hasValue = Boolean(String(e.target.value || '').trim());
        const valid = !hasValue || isValidIsoDate(parsed);
      ...
      input.value = normalizeDateForInput(input.value);
    });
  }
  ...
  initEvents() {
    ...
    document.getElementById('btn-header-signout').addEventListener('click', () => this.signOutSystem());
    document.getElementById('btn-header-back-tab').addEventListener('click', () => this.goBackTab());
    ...
    const btnOpenAgendaWhatsApp = document.getElementById('btn-open-agenda-whatsapp');
    if (btnOpenAgendaWhatsApp) btnOpenAgendaWhatsApp.addEventListener('click', () => this.switchTab('agenda'));

    const btnOpenBirthdays = document.getElementById('btn-open-birthdays');
    if (btnOpenBirthdays) {
      btnOpenBirthdays.addEventListener('click', () => this.openBirthdaysSection());
    }

    const btnOpenBirthdaysHeader = document.getElementById('btn-open-birthdays-header');
    if (btnOpenBirthdaysHeader) {
      btnOpenBirthdaysHeader.addEventListener('click', () => this.openBirthdaysSection());
    }

    const btnSaveWhatsAppTemplates = document.getElementById('btn-save-whatsapp-templates');
    if (btnSaveWhatsAppTemplates) {
      btnSaveWhatsAppTemplates.addEventListener('click', () => this.saveWhatsAppTemplates());
    }

    const btnRestoreConfirmTemplate = document.getElementById('btn-restore-whatsapp-template-confirmation');
    if (btnRestoreConfirmTemplate) {
      btnRestoreConfirmTemplate.addEventListener('click', () => this.restoreWhatsAppTemplate('confirmation'));
    }

    const btnRestoreBirthdayTemplate = document.getElementById('btn-restore-whatsapp-template-birthday');
    if (btnRestoreBirthdayTemplate) {
      btnRestoreBirthdayTemplate.addEventListener('click', () => this.restoreWhatsAppTemplate('birthday'));
    }

    const btnCloseWhatsAppSend = document.getElementById('btn-close-whatsapp-send');
    const btnCancelWhatsAppSend = document.getElementById('btn-cancel-whatsapp-send');
    if (btnCloseWhatsAppSend) btnCloseWhatsAppSend.addEventListener('click', () => this.closeWhatsAppSendModal());
    if (btnCancelWhatsAppSend) btnCancelWhatsAppSend.addEventListener('click', () => this.closeWhatsAppSendModal());

    const modeInputs = document.querySelectorAll('input[name="whatsapp-send-mode"]');
    modeInputs.forEach((radio) => {
      radio.addEventListener('change', () => this.updateWhatsAppSendModeUI());
    });

    const btnConfirmWhatsAppSend = document.getElementById('btn-confirm-whatsapp-send');
    if (btnConfirmWhatsAppSend) {
      btnConfirmWhatsAppSend.addEventListener('click', () => this.confirmWhatsAppSend());
    }

    const birthdayDaysInput = document.getElementById('birthday-reminder-days');
    if (birthdayDaysInput) {
      birthdayDaysInput.addEventListener('change', (e) => {
        const val = this.updateBirthdayReminderDays(e.target.value);
        if (val !== null) {
          this.showToast(`Aviso de aniversarios ajustado para ${val} dia(s) de antecedencia.`, 'info');
        }
      });
    }

    const headerBirthdayDaysInput = document.getElementById('header-birthday-reminder-days');
    if (headerBirthdayDaysInput) {
      headerBirthdayDaysInput.addEventListener('change', (e) => {
        const val = this.updateBirthdayReminderDays(e.target.value);
        if (val !== null) {
          this.showToast(`Aviso de aniversarios ajustado para ${val} dia(s) de antecedencia.`, 'info');
        }
    ...
    const btnAgendaList = document.getElementById('btn-agenda-list');
    const btnAgendaCalendar = document.getElementById('btn-agenda-calendar');
    const btnPrevWeek = document.getElementById('btn-agenda-prev-week');
    const btnCurrentWeek = document.getElementById('btn-agenda-current-week');
    const btnNextWeek = document.getElementById('btn-agenda-next-week');

    if (btnAgendaList) {
      btnAgendaList.addEventListener('click', () => {
        this.agendaView = 'list';
        this.renderAgendaViewMode();
      });
    }

    if (btnAgendaCalendar) {
      btnAgendaCalendar.addEventListener('click', () => {
    ...
    const btnExportPdfReport = document.getElementById('btn-export-pdf-report');
    if (btnExportPdfReport) btnExportPdfReport.addEventListener('click', () => this.exportReportPdf());
  ...
  switchTab(tabId, options = {}) {
    ...
    if (!skipHistory && this.activeTab && this.activeTab !== tabId) {
  ...
  goBackTab() {
    if (!this.tabHistory.length) {
      this.showToast('Nenhuma aba anterior para retornar.', 'info');
      return;
    }
  ...
  signOutSystem() {
    const confirmed = confirm('Deseja sair do sistema agora? Seus dados locais continuarão salvos.');
    if (!confirmed) return;

    // Fecha modais e volta para visão inicial do dashboard.
    document.querySelectorAll('.modal-backdrop').forEach((modal) => modal.classList.remove('active'));
    this.tabHistory = [];
    this.switchTab('dashboard', { skipHistory: true });

    // Encerra apenas a sessão atual de sincronização em nuvem.
    ...
    this.showToast('Sessão encerrada. Você saiu do sistema.', 'info');
  }
  ...
  renderWhatsAppBirthdays() {
    const container = document.getElementById('whatsapp-birthdays-list');
    const summary = document.getElementById('whatsapp-birthdays-summary');
    if (!container) return;

    const today = new Date();
    const reminderDays = Number.isFinite(this.birthdayReminderDays) ? this.birthdayReminderDays : 7;

    const birthdays = this.clients
      .filter(client => client.dob)
      .map(client => ({
        client,
        date: new Date(parseDateBR(client.dob))
      }))
      .filter(entry => !Number.isNaN(entry.date.getTime()))
      .map((entry) => {
        const [year, month, day] = String(parseDateBR(entry.client.dob)).split('-').map(Number);
        ...
        const daysUntil = Math.round((nextBirthday - todayStart) / msPerDay);
        return {
          ...entry,
          nextBirthday,
          daysUntil
    ...
    const dueList = birthdays.filter(entry => entry.daysUntil <= reminderDays);
    this.updateDashboardBirthdayNotice(dueList.length, reminderDays);

    if (summary) {
      summary.textContent = `Avisando aniversarios com ate ${reminderDays} dia(s) de antecedencia.`;
    }
    ...
    if (dueList.length === 0) {
      container.innerHTML = `
        <div class="empty-state">
          ...
          <h4>Nenhum aniversario no prazo de aviso</h4>
          <p>Ajuste os dias de antecedencia ou aguarde proximos aniversarios.</p>
        </div>
      `;
      if (window.lucide) {
        window.lucide.createIcons();
    ...
    container.innerHTML = dueList.map(({ client, nextBirthday, daysUntil }) => {
      const birthdayLabel = formatDateBR(nextBirthday.toISOString().slice(0, 10));
      const reminderText = daysUntil === 0
        ? 'Aniversaria hoje'
        : (daysUntil === 1 ? 'Aniversaria amanha' : `Aniversaria em ${daysUntil} dias`);
      return `
        <div class="pending-mini-item birthday-mini-item" style="align-items:center;">
          <div class="pending-mini-info">
            <h4>${client.name}</h4>
            <p>${birthdayLabel} • ${client.phone || 'Sem telefone'}</p>
            <small class="birthday-reminder-tag">${reminderText}</small>
          </div>
          <div class="birthday-actions" style="text-align:right;">
            <button class="btn btn-sm btn-success" onclick="app.sendBirthdayWhatsApp('${client.id}')">
              <i data-lucide="message-circle"></i> WhatsApp
            </button>
            <button class="btn btn-sm btn-secondary" onclick="app.openClientModal('${client.id}')">Editar</button>
          </div>
        </div>
      `;
    }).join('');
  ...
  updateDashboardBirthdayNotice(dueCount, reminderDays) {
    const reminderText = document.getElementById('header-birthday-reminder-days');
    const countText = document.getElementById('header-birthday-reminder-count');
    if (reminderText) reminderText.value = String(reminderDays);
    if (!countText) return;

    countText.textContent = `${dueCount} aniversariante(s) no prazo.`;
    countText.classList.toggle('birthday-count-alert', dueCount > 0);
  }

  updateBirthdayReminderDays(rawValue) {
    let val = parseInt(rawValue, 10);
    if (Number.isNaN(val) || val < 0) val = 7;
    if (val > 90) val = 90;

    this.birthdayReminderDays = val;
    localStorage.setItem('consultorio_birthday_reminder_days', String(val));

    const birthdayDaysInput = document.getElementById('birthday-reminder-days');
    const headerBirthdayDaysInput = document.getElementById('header-birthday-reminder-days');
    if (birthdayDaysInput) birthdayDaysInput.value = String(val);
    if (headerBirthdayDaysInput) headerBirthdayDaysInput.value = String(val);
  ...
  saveWhatsAppTemplates() {
    const confirmationInput = document.getElementById('whatsapp-template-confirmation');
    const birthdayInput = document.getElementById('whatsapp-template-birthday');
    if (!confirmationInput || !birthdayInput) return;

    const confirmation = confirmationInput.value.trim();
    const birthday = birthdayInput.value.trim();
    if (!confirmation || !birthday) {
      this.showToast('Preencha os dois textos de WhatsApp antes de salvar.', 'warning');
      return;
    }
    ...
    localStorage.setItem('consultorio_whatsapp_template_confirmation', confirmation);
    localStorage.setItem('consultorio_whatsapp_template_birthday', birthday);
    this.showToast('Textos de WhatsApp salvos com sucesso!', 'success');
  }

  restoreWhatsAppTemplate(type) {
    const confirmationInput = document.getElementById('whatsapp-template-confirmation');
    const birthdayInput = document.getElementById('whatsapp-template-birthday');

    if (type === 'confirmation' && confirmationInput) {
      confirmationInput.value = DEFAULT_WHATSAPP_CONFIRM_TEMPLATE;
      this.showToast('Texto de confirmação restaurado para o padrão.', 'info');
      return;
    }

    if (type === 'birthday' && birthdayInput) {
      birthdayInput.value = DEFAULT_WHATSAPP_BIRTHDAY_TEMPLATE;
      this.showToast('Texto de aniversário restaurado para o padrão.', 'info');
    }
  }

  applyTemplate(template, dataMap) {
  ...
  sendBirthdayWhatsApp(clientId) {
    ...
    const template = this.whatsappTemplates.birthdayGreeting || DEFAULT_WHATSAPP_BIRTHDAY_TEMPLATE;
    const msg = this.applyTemplate(template, {
      nome: client.name || 'cliente',
      profissional: APP_BRAND_NAME,
      telefone: client.phone || ''
    });

    this.openWhatsAppSendModal({
  ...
  openBirthdaysSection() {
    ...
    const focusTarget = container.querySelector('button, [role="button"], a, input, select, textarea') || container;
  ...
  renderDashboard() {
    ...
    if (rangeApps.length === 0) {
      ...
      todayListContainer.innerHTML = sortedPeriod.slice(0, 5).map(app => {
        ...
        const itemAriaLabel = `Editar consulta de ${app.clientName} em ${formatDateBR(app.date)} as ${app.time}`;
        return `
          <div class="appt-compact-item appt-compact-item-action" role="button" tabindex="0" aria-label="${itemAriaLabel}" onclick="app.editAppointment('${app.id}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();app.editAppointment('${app.id}');}">
    ...
    this.updateDashboardAccessibility({
      appointmentsCount: rangeApps.length,
      completedInPeriod,
      totalReceived,
      totalPending,
    ...
    if (pendingApps.length === 0) {
      ...
      pendingListContainer.innerHTML = pendingApps.slice(0, 4).map(app => {
        ...
        const pendingAriaLabel = `Abrir pagamento de ${app.clientName}. Consulta em ${formatDateBR(app.date)}. Valor pendente ${formatCurrency(rest)}`;
        return `
          <div class="pending-mini-item pending-mini-item-action" role="button" tabindex="0" aria-label="${pendingAriaLabel}" onclick="app.openQuickPayModal('${app.id}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();app.openQuickPayModal('${app.id}');}">
            <div class="pending-mini-info">
              <h4>${app.clientName}</h4>
            ...
            <div style="text-align: right;">
              ...
              <button class="btn btn-sm btn-success" style="margin-top: 4px;" onclick="event.stopPropagation(); app.openQuickPayModal('${app.id}')">
                Registrar Pagamento
              </button>
            </div>
          </div>
  ...
  updateDashboardAccessibility(stats) {
    ...
    const configs = [
      {
        tabId: 'agenda',
        label: `Consultar agenda. ${stats.appointmentsCount} consultas no periodo, ${stats.completedInPeriod} concluidas.`
      },
      {
        tabId: 'financeiro',
        label: `Abrir financeiro. Recebido no periodo: ${formatCurrency(stats.totalReceived)}.`
      },
      {
        tabId: 'financeiro',
        label: `Abrir financeiro para pendencias. Total em aberto: ${formatCurrency(stats.totalPending)} em ${stats.pendingCount} cobrancas.`
      },
      {
        tabId: 'clientes',
        label: `Abrir clientes. Total de ${stats.clientsCount} clientes cadastrados.`
      }
    ];
    ...
    cards.forEach((card, index) => {
      ...
      card.setAttribute('title', 'Pressione Enter para abrir');

      if (card.dataset.a11yBound === 'true') return;

      card.addEventListener('click', () => this.switchTab(cfg.tabId));
      card.addEventListener('keydown', (event) => {
        if (event.key === 'Enter' || event.key === ' ') {
          event.preventDefault();
          this.switchTab(cfg.tabId);
        }
      });
      ...
      return `
        <tr>
          ...
          <td style="text-align: right;">
            ...
            <button class="btn btn-sm btn-secondary" title="Enviar confirmação via WhatsApp" onclick="app.sendAppointmentWhatsApp('${app.id}')">
              <i data-lucide="message-circle"></i> WhatsApp
            </button>
  ...
  renderAgendaViewMode() {
    const listView = document.getElementById('agenda-list-view');
    const calendarView = document.getElementById('agenda-calendar-view');
    const btnList = document.getElementById('btn-agenda-list');
    const btnCalendar = document.getElementById('btn-agenda-calendar');

    if (!listView || !calendarView || !btnList || !btnCalendar) return;

    const isCalendar = this.agendaView === 'calendar';
    listView.style.display = isCalendar ? 'none' : 'block';
    calendarView.style.display = isCalendar ? 'block' : 'none';

    btnList.classList.toggle('active', !isCalendar);
  ...
  renderAgendaCalendar() {
    ...
    monday.setDate(today.getDate() + mondayOffset + (this.agendaWeekOffset * 7));
    ...
    const startLabel = weekDays[0].toLocaleDateString('pt-BR');
    const endLabel = weekDays[4].toLocaleDateString('pt-BR');
    rangeLabel.textContent = `${startLabel} até ${endLabel}`;

    const weekdayNames = ['Seg', 'Ter', 'Qua', 'Qui', 'Sex'];
    let html = '<div class="agenda-header blank"></div>';
    weekDays.forEach((day, idx) => {
      html += `
        <div class="agenda-header">
          <span>${weekdayNames[idx]}</span>
          <span class="agenda-header-date">${day.toLocaleDateString('pt-BR')}</span>
        </div>
      `;
    });

    for (let hour = 7; hour <= 19; hour++) {
      html += `<div class="agenda-time-axis">${String(hour).padStart(2, '0')}:00</div>`;

      weekDays.forEach(day => {
        const dayKey = `${day.getFullYear()}-${String(day.getMonth() + 1).padStart(2, '0')}-${String(day.getDate()).padStart(2, '0')}`;
        const events = this.appointments
          .filter(app => app.date === dayKey && parseInt((app.time || '00:00').split(':')[0], 10) === hour)
          .sort((a, b) => (a.time || '').localeCompare(b.time || ''));

        if (!events.length) {
          html += '<div class="agenda-cell"></div>';
          return;
        }

        html += `<div class="agenda-cell">${events.map(app => {
          const payClass = (app.paymentStatus || '').toLowerCase();
          return `
            <div class="agenda-event agenda-event-${payClass}" role="button" tabindex="0" onclick="app.editAppointment('${app.id}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();app.editAppointment('${app.id}');}">
              <div class="agenda-event-time">${app.time}</div>
              <div class="agenda-event-title-row">
                <div class="agenda-event-title">${app.clientName}</div>
                <button class="agenda-event-whatsapp" title="Enviar confirmação no WhatsApp" onclick="event.stopPropagation(); app.sendAppointmentWhatsApp('${app.id}')">
                  <i data-lucide="message-circle"></i>
                </button>
              </div>
              <div class="agenda-event-procedure">${app.procedure}</div>
              <div class="agenda-event-payment">${app.paymentStatus}</div>
            </div>
          `;
        }).join('')}</div>`;
      });
    ...
    // Linha extra para avisos de aniversariantes da semana
    html += '<div class="agenda-time-axis">Avisos</div>';
    weekDays.forEach(day => {
      const month = day.getMonth() + 1;
      const date = day.getDate();
      const birthdays = this.clients.filter(c => {
        if (!c.dob) return false;
        const [y, m, d] = String(parseDateBR(c.dob)).split('-').map(Number);
        if (!m || !d) return false;
      ...
      html += `<div class="agenda-cell">${birthdays.map(c => `
        <div class="agenda-birthday-alert">
          <i data-lucide="cake"></i> Aniversário de ${c.name.split(' ')[0]}
        </div>
      `).join('')}</div>`;
    });
  ...
  renderFinanceiroTable() {
    ...
    this.updateFinanceCardsAccessibility({
      grandTotal,
      paidTotal,
      pendingTotal,
      paidCount,
  ...
  updateFinanceCardsAccessibility(stats) {
    const cards = document.querySelectorAll('#tab-financeiro .fin-summary-grid .fin-card');
    if (!cards.length) return;

    const configs = [
      {
        filter: 'todos',
        label: `Mostrar todos os lancamentos para editar. Total bruto no periodo: ${formatCurrency(stats.grandTotal)}.`
      },
      {
        filter: 'pago',
        label: `Mostrar consultas pagas para editar. Total recebido: ${formatCurrency(stats.paidTotal)} em ${stats.paidCount} pagamento(s).`
      },
      {
        filter: 'pendente',
        label: `Mostrar lancamentos pendentes para editar. Total em aberto: ${formatCurrency(stats.pendingTotal)} em ${stats.pendingCount} lancamento(s).`
      }
    ];
    ...
    cards.forEach((card, index) => {
      ...
      card.setAttribute('tabindex', '0');
      card.setAttribute('aria-label', cfg.label);
      card.setAttribute('title', 'Pressione Enter para filtrar e editar');

      if (card.dataset.financeA11yBound === 'true') return;

      card.addEventListener('click', () => this.openFinanceFilterForEdit(cfg.filter));
      card.addEventListener('keydown', (event) => {
        if (event.key === 'Enter' || event.key === ' ') {
          event.preventDefault();
          this.openFinanceFilterForEdit(cfg.filter);
        }
      });

      card.dataset.financeA11yBound = 'true';
    });
  }

  openFinanceFilterForEdit(filter) {
    this.finFilter = filter;
    document.querySelectorAll('[data-fin-filter]').forEach(btn => {
      const isActive = btn.getAttribute('data-fin-filter') === filter;
      btn.classList.toggle('active', isActive);
    });

    this.renderFinanceiroTable();

    const firstEditButton = document.querySelector('#financeiro-table-body button[onclick*="editAppointment"]');
    if (firstEditButton) {
      firstEditButton.focus();
      return;
    }

    this.showToast('Nenhum lancamento disponivel para editar neste filtro.', 'info');
  }
  ...
  openClientModal(clientId = null) {
    ...
    if (clientId) {
      ...
      if (cli) {
        ...
        document.getElementById('client-dob').value = cli.dob ? formatDateBR(cli.dob) : '';
  ...
  saveClientForm() {
    ...
    const dob = parseDateBR(dobInput.value);
    ...
    const hasDob = Boolean(String(dobInput.value || '').trim());
    if (hasDob && !isValidIsoDate(dob)) {
  ...
  saveAppointmentForm() {
    ...
    const date = parseDateBR(dateInput.value);
    ...
    if (!isValidIsoDate(date)) {
      ...
      this.showToast('Data da consulta inválida. Use o formato dd/mm/aaaa.', 'warning');
      return;
    }
  ...
  sendAppointmentWhatsApp(appointmentId) {
    ...
    const template = this.whatsappTemplates.appointmentConfirmation || DEFAULT_WHATSAPP_CONFIRM_TEMPLATE;
    const text = this.applyTemplate(template, {
      nome: (client && client.name) || appointment.clientName || 'cliente',
    ...
    this.openWhatsAppSendModal({
      recipientName: (client && client.name) || appointment.clientName || 'cliente',
      recipientPhone: phone,
      message: text,
  ...
  openWhatsAppSendModal(payload) {
    this.pendingWhatsAppSend = payload;
    ...
    const preview = document.getElementById('whatsapp-send-preview');
    const testPhoneInput = document.getElementById('whatsapp-test-phone');
    const normalMode = document.querySelector('input[name="whatsapp-send-mode"][value="normal"]');

    if (!modal || !recipientInput || !preview || !normalMode) return;

    recipientInput.value = `${payload.recipientName} (${payload.recipientPhone})`;
    preview.value = payload.message || '';
    if (testPhoneInput) testPhoneInput.value = '';
    normalMode.checked = true;
    this.updateWhatsAppSendModeUI();
  ...
  updateWhatsAppSendModeUI() {
    const selected = document.querySelector('input[name="whatsapp-send-mode"]:checked');
    const testGroup = document.getElementById('whatsapp-test-phone-group');
    if (!selected || !testGroup) return;
    testGroup.style.display = selected.value === 'teste' ? 'block' : 'none';
  }

  confirmWhatsAppSend() {
    if (!this.pendingWhatsAppSend) return;

    const selected = document.querySelector('input[name="whatsapp-send-mode"]:checked');
    const mode = selected ? selected.value : 'normal';
    let targetPhone = this.pendingWhatsAppSend.recipientPhone;

    if (mode === 'teste') {
      const testPhoneInput = document.getElementById('whatsapp-test-phone');
      const normalized = this.normalizeWhatsAppPhone(testPhoneInput ? testPhoneInput.value : '');
      if (!normalized) {
        this.showToast('Informe um telefone válido para teste.', 'warning');
        return;
      }
      targetPhone = normalized;
    }

    window.open(`https://wa.me/${targetPhone}?text=${encodeURIComponent(this.pendingWhatsAppSend.message || '')}`, '_blank', 'noopener');
    this.showToast(mode === 'teste' ? 'Mensagem de teste preparada no WhatsApp.' : 'Mensagem preparada no WhatsApp.', 'success');
  ...
  saveExpenseForm() {
    ...
    if (!isValidIsoDate(date)) {
      dateInput.classList.add('is-invalid');
      dateInput.setAttribute('aria-invalid', 'true');
      this.showToast('Data da despesa inválida. Use o formato dd/mm/aaaa.', 'warning');
      return;
    }
    dateInput.classList.remove('is-invalid');
    dateInput.setAttribute('aria-invalid', 'false');
  ...
  generateReport(type) {
    ...
    let report = '';
    const reportTitles = {
      paciente: 'Relatório por Paciente',
    ...
    this.lastGeneratedReportTitle = reportTitles[type] || 'Relatório';
  ...
  async exportReportPdf() {
    const output = document.getElementById('report-output');
    if (!output || !output.value) {
      this.showToast('Gere um relatório antes de exportar em PDF.', 'warning');
      return;
    }

    if (!window.jspdf || !window.jspdf.jsPDF) {
      this.showToast('Biblioteca de PDF não carregada.', 'danger');
      return;
    }
    ...
    const margin = 42;
    let cursorY = margin;
    const reportTitle = this.lastGeneratedReportTitle || 'Relatório';

    try {
      const logoDataUrl = await this.loadImageAsDataUrl('logo-clinica.svg');
      if (logoDataUrl) {
        doc.addImage(logoDataUrl, 'PNG', margin, cursorY, 86, 48);
      }
    } catch (e) {
      // Se a imagem falhar, segue sem logo para não bloquear exportação.
    }

    doc.setFont('helvetica', 'bold');
    doc.setFontSize(16);
    doc.text(APP_BRAND_NAME, margin + 96, cursorY + 18);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    doc.text(APP_BRAND_SUBTITLE, margin + 96, cursorY + 34);
    ...
    const lines = doc.splitTextToSize(output.value, pageWidth - margin * 2);
    lines.forEach((line) => {
    ...
    doc.save(`${safeTitle}-${getTodayStr()}.pdf`);
      ...
      <html>
        ...
        <body>
          <div class="report-header">
            <img src="logo-clinica.svg" class="report-logo" alt="Logo">
            <div>
